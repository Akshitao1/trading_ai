...
export const useCalculatorLogic = () => {
  ...
  const calculatePredictions = async (inputs: TradingInputs): Promise<PredictionResults> => {
    ...
    try {
      ...
      const apiUrl = `https://trading-ai-7sam.onrender.com/api/cpas-for-budget?budget=${inputs.budget}&duration=${weeks}&start_date=${inputs.startDate}&end_date=${inputs.endDate}`;
